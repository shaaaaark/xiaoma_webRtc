<template>
  <div style="width: 98%; height: 98%; margin-top: 20px">
    ...
    <el-row>
      <el-button type="primary" @click="backToIndex">返回主界面</el-button>
    </el-row>
  </div>
</template>
...
function initInnerLocalDevice() {
  ...
  navigator.mediaDevices
    ...
    .then(function (stream) {
      ...
      /**
       * Enumerates the available media devices and populates the localDevice object with the audio and video input/output devices.
       *
       * @returns {Promise} A promise that resolves with the updated localDevice object.
       */
      navigator.mediaDevices
        .enumerateDevices()
    ...
    async getTargetDeviceMedia(videoId, audioId) {
      const constraints = {
        ...
        video: {
          ...
          frameRate: { ideal: this.formInline.frameRate, max: 60 }
        }
      }
      if (window.stream) {
